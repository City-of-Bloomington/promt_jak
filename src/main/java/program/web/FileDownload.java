...
public class FileDownload extends TopServlet{
	...
	try{
		...
		filePath = server_path+fup.getYear()+"/"+fup.getImageThunbName();
		doDownloadThumb(req, res, filePath, fup);
		return;
	    }
	}catch(Exception ex){
    ...
    void doDownloadThumb(HttpServletRequest request,
			 HttpServletResponse response,
			 String filePath,
			 MediaFile fup){
